...
export default function Page() {
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      <div className="top-strip"><div className="container flex items-center justify-between gap-3"><span>La plataforma de inglés para avanzar de verdad</span><a href="#community">Únete a nuestra comunidad <ArrowRight aria-hidden="true" /></a></div></div>
      <header className="site-header"><div className="container header-inner">
        ...
        <nav className="desktop-nav" aria-label="Navegación principal">
          <a href="#courses">Cursos <ChevronDown aria-hidden="true" /></a><a href="#lessons">Lecciones</a><a href="#practice">Practica</a><a href="#resources">Recursos</a>
        </nav>
        <div className="header-actions"><button className="icon-button" aria-label="Buscar" onClick={() => document.getElementById('search')?.focus()}><Search aria-hidden="true" /></button><button className="language">ES <ChevronDown aria-hidden="true" /></button><a className="login-link" href="#login">Entrar</a><a className="button button-small" href="#signup">Crear cuenta</a><button className="menu-button" aria-label={menuOpen ? 'Cerrar menú' : 'Abrir menú'} onClick={() => setMenuOpen(!menuOpen)}>{menuOpen ? <X /> : <Menu />}</button></div>
      ...
      <section className="hero" id="top"><div className="container hero-grid"><div className="hero-copy"><div className="eyebrow"><span className="eyebrow-dot" /> Aprende. Practica. Avanza.</div><h1>Tu inglés no tiene <em>límites.</em></h1><p>Lecciones claras, práctica real y una comunidad que te acompaña. Aprende inglés a tu ritmo y úsalo desde el primer día.</p><form className="search-form" onSubmit={submitSearch}><Search aria-hidden="true" /><input id="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="¿Qué quieres aprender hoy?" aria-label="Buscar una lección" /><button type="submit" className="button">Buscar</button></form>{searchMessage && <p className="form-message" role="status">{searchMessage}</p>}<div className="hero-links"><span>Empieza por:</span><a href="#courses">Inglés básico</a><a href="#practice">Pronunciación</a><a href="#lessons">Gramática</a></div></div><aside className="daily-card"><div className="daily-top"><span className="pill orange-pill">LECCIÓN DEL DÍA</span><span className="streak"><Trophy aria-hidden="true" /> 7 días</span></div><div className="daily-art"><span className="speech-mark">“</span><span className="art-word">Actually</span><span className="art-meaning">/ˈæktʃuəli/ · en realidad</span></div><p className="daily-label">EXPRESIÓN DE HOY</p><h2>“Actually” no significa actualmente</h2><p>Descubre cómo usar una de las palabras que más confunden a los estudiantes.</p><a className="text-link" href="#lessons">Ver lección <ArrowRight aria-hidden="true" /></a></aside></div></section>
